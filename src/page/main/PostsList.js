...
const PostsList = memo(({ 
  ...
  // Очистка дубликатов с сохранением ВСЕХ глубоких свойств объекта поста (включая опрос)
  ...
  const renderItem = useCallback((item, index) => {
    ...
    return (
      <Box key={`p-${item._id || index}`} sx={{ mb: 0 }}>
        {/* Компонент получает обновленный стейт и корректно перерисовывает опрос на главной */}
