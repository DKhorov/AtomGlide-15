...
// Выносим константы и стили за пределы компонента для предотвращения лишних рендеров
...
// Вынесите этот гигантский массив в отдельный файл constants.js!
...
const PostWithComments = memo(({ post, onDelete, onPostUpdate, isFullPost }) => {
  ...
  // Оптимизированные стили для модалки репортов (мемоизация)
  ...
  const toggleReaction = async (emoji) => {
    ...
    try {
      const token = window.localStorage.getItem('token');
      const { data } = await axios.post(`${CDN_URL}/post/${post._id}/reaction`, { emoji }, { headers: { Authorization: `Bearer ${token}` } });
      if (data.success) setReactions(data.reactions);
    } catch (err) { console.error(err); }
    handleClosePicker();
  };

  // ИСПРАВЛЕНО: Теперь репорт действительно отправляется на сервер
    ...
    return createPortal(
      <div style={modalOverlayStyle} onClick={handleCloseAuthModal}>
        <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
          ...
          <Typography sx={{ color: '#aaaaaa', fontSize: '0.85rem', mb: 3 }}>Войдите, чтобы оставлять комментарии и ставить реакции.</Typography>
    ...
    <Paper elevation={0} sx={{ p: 2.5, borderRadius: '10px', mb: 1, bgcolor: '#1a1b1e', border: '1px solid #2a2a2a' }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mt: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          <IconButton onClick={handleOpenShareModal} sx={{ bgcolor: '#25262b', color: '#ffffff', borderRadius: '12px' }}><ShareOutlined sx={{ fontSize: 20 }} /></IconButton>
        </Stack>
      </Stack>
